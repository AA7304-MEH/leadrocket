...
export class LeadGenerationService {
  ...
  static async generateLeads(user: IUser, count: number = 5): Promise<LeadGenerationData[]> {
  ...
  static async saveGeneratedLeads(user: IUser, leads: LeadGenerationData[]): Promise<any[]> {
    const savedLeads = [];
  ...
  static async getLeadStats(user: IUser) {
    const totalLeads = await Lead.countDocuments({ user: user._id });
    const newLeads = await Lead.countDocuments({ user: user._id, status: 'new' });
    const qualifiedLeads = await Lead.countDocuments({ user: user._id, status: 'qualified' });
    const convertedLeads = await Lead.countDocuments({ user: user._id, status: 'converted' });
